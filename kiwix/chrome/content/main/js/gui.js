...
function manageOpenFile(path, noSearchIndexCheck) {
    ...
    /* Load ZIM file */
        ...
        if (zimAccessor) {
            ...
            addOneShotEventListener(getHtmlRenderer(), "pageshow", function() {
                managePurgeHistory()
            }, true);
...
function initUserInterface() {
    ...
    /* Apply UI settings */
    if (settings.displayStatusBar() != undefined) {
        changeStatusBarVisibilityStatus(settings.displayStatusBar());
    }
    if (settings.displayFullScreen() != undefined) {
        if (settings.displayFullScreen()) {
            UIToggleFullScreen();
        }
    }
    if (settings.displayBookmarksBar() === true) {
        UIToggleBookmarksBar();
    }
    if (settings.invertedColors() === true) {
        toggleInvertedColors(true);
    }
...
function initHtmlRendererEventListeners(id) {
    ...
    htmlRenderer.addEventListener("pageshow", function() {
        updateTabHeader(id)
    }, true);
    htmlRenderer.addEventListener("load", function() {
        updateTabHeader(id);
        applyInitScroll(this);
    }, true);
    ...
    if (initScrollY > 0) {
        if (areColorsInverted() && browser.contentDocument.getElementsByTagName("iframe").length > 0) {
            ...
            iframe.onload = function() {
                iframe.contentWindow.scrollTo(0, 420);
            };
        } else {
...
function initEventListeners() {
    ...
    document.addEventListener("onZimArg", function(e) {
        loadContentFromCommandLine(e.data)
    }, false);
}
